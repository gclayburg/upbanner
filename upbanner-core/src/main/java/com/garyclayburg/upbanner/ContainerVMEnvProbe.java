...
import com.garyclayburg.upbanner.jarprobe.NullJarDumper;
...
public class ContainerVMEnvProbe {
    ...
    private UpbannerSettings upbannerSettings;

    public ContainerVMEnvProbe(UpbannerSettings upbannerSettings) {
    ...
    public JarProbe jarDumper() {
        ...
        } else { //avoid probing env to pick correct Jar dumper if we will never use it
            return new NullJarDumper();
        }
    }
    ...
    public JarProbe jarDumperMissing() {
        log.debug("JarFile is not in classpath. We are not running from jar");
        return new FileJarDumper();
    }
}
